...
function mapCategoryToOutputFilter(category: Category): OutputFilter {
  switch (category) {
    case "대학생": return "STUDENT";
    case "직장인": return "WORKER";
    case "온라인": return "ONLINE";
    case "동아리": return "CLUB";
    case "모임": return "MEETING";
    case "대면": return "OFFLINE";
    case "전체":
    default: return "ALL";
  }
}
...
function mapApplyType(myStatus: string): "No" | "Wait" | "Yes" {
  if (myStatus === "NONE" || myStatus === "WITHDRAWN" || myStatus === "KICKED") return "No";
  if (myStatus === "MEMBER" || myStatus === "JOINED" || myStatus === "STAFF" || myStatus === "OWNER") return "Yes";
  return "Wait";
}
...
export default function GroupsPageClient() {
  ...
  const recommendationClubs: ClubSummary[] = useMemo(() => {
    return (recData?.recommendations ?? []).map(mapRecommendationItem);
  }, [recData]);
  ...
  const onSubmitApply = async (clubId: number, reason: string) => {
    if (!reason.trim() || joinPending) return;
    try {
      const msg = await joinAsync({ clubId, body: { joinMessage: reason.trim() } });
      toast.success(msg);
      setApplyClubId(null);
    } catch (e: any) {
  ...
  const onSubmitSearch = () => {
    ...
    if (!keyword) { setAppliedParams(null); return; }
    setAppliedParams({
